...
//		private static const OBJECT_TYPE_BIT:int = 19;
      ...
      public var specularEnabled:Boolean = true;
		...
		override alternativa3d function fillResources(resources:Dictionary, resourceType:Class):void {
         ...
         if (colorMap != null &&
					A3DUtils.checkParent(getDefinitionByName(getQualifiedClassName(colorMap)) as Class, resourceType)) {
				resources[colorMap] = true;
			}

         if (surfaceMap != null &&
					A3DUtils.checkParent(getDefinitionByName(getQualifiedClassName(surfaceMap)) as Class, resourceType)) {
				resources[surfaceMap] = true;
			}
...
//			fragmentLinker.setInputParams(omniMulShadowProcedure, "tNormal", "tViewVector", "tTotalLight", "cAmbientColor");
		 ...
		 * @param lightsLength
		 */
		private function getProgram(object:Object3D, programs:Array, camera:Camera3D, materialKey:int, opacityMap:TextureResource, alphaTest:int, lightsGroup:Vector.<Light3D>, lightsLength:int, isFirstGroup:Boolean, shadowedLight:Light3D):TankMaterialProgram {
			// 0 bit - lightmap
			...
			var program:TankMaterialProgram = programs[key];
			...
			if (program == null) {
				...
				if (isFirstGroup){
					...
					fragmentLinker.setInputParams(_ambientLightProcedure, "cAmbientColor", "tTotalLight");
...
//				}
		...
		private function addDrawUnits(program:TankMaterialProgram, camera:Camera3D, surface:Surface, geometry:Geometry, opacityMap:TextureResource, lights:Vector.<Light3D>, lightsLength:int, isFirstGroup:Boolean, shadowedLight:Light3D, opaqueOption:Boolean, transparentOption:Boolean, objectRenderPriority:int):void {
			// Buffers
         ...
         drawUnit.setTextureAt(program.sColor, colorMap._texture);
         drawUnit.setTextureAt(program.sSurface, surfaceMap._texture);
...
//			}
		...
		override alternativa3d function collectDraws(camera:Camera3D, surface:Surface, geometry:Geometry, lights:Vector.<Light3D>, lightsLength:int, useShadow:Boolean, objectRenderPriority:int = -1):void {
			...
			// Iterate groups
			var materialKey:int;
			var program:TankMaterialProgram;
		...
		override protected function clonePropertiesFrom(source:Material):void {
			...
			var sMaterial:TankMaterial = TankMaterial(source);
...
class TankMaterialProgram extends ShaderProgram {
   ...
   public var cDiffuseStrength:int = -1;
	...
	public function TankMaterialProgram(vertex:Linker, fragment:Linker, numLigths:int) {
